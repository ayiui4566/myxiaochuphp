...
class ThumbHandler {
        ...
        var $img_display_quality = 90; // 图片jpg显示质量1-100,png的不用传质量
               ...
               // echo '$img_type====='.$img_type;
                ...
                if (function_exists ( "file_get_contents" )) {
                    ...
                    $context['http'] = array
                        ...
                        $src = file_get_contents ( $src_img,false, stream_context_create($context) );
                } else {
                ...
                if(!$this->h_src)
                {
                    echo "@ImageCreateFromString出现问题";
                }
        ...
        /**
         * 创建图片,主函数
         * @param    integer    $a     当缺少第二个参数时，则根据此值（像素值）作为宽高最大值等比缩放即宽和高最大为$a
         *                             否则作为宽度值
        ...
        function _setNewImgSize($img_w, $img_h = null) {
                $num = func_num_args ();
                if (1 == $num) {
                        $this->img_scale = $img_w; // 宽度/原图片宽作为比例等比缩放
                        
                        $bili = $this->src_w/$this->src_h;
                      
                        if($bili<1)
                        {//width<height
                                $this->fill_w = $img_w*$bili;
                                $this->fill_h = $img_w;
                        }else{//width>height
                                $this->fill_w = $img_w;
                                $this->fill_h = $img_w*$bili;
                        }
                        //$this->fill_w = $this->img_scale - $this->img_border_size * 2;
                        //$this->fill_h = round ( $this->src_h * $this->img_scale / $this->src_w ) - $this->img_border_size * 2;
