...
class Logs{
    ...
    public function pushSqlStr($sql){
        if(in_array($sql,$this->sqlAry)){
            $this->setLog("查询出一条相同的语句：".$sql);
        }
        $this->setLog("sql:".$sql);
        $this->sqlAry[] = $sql;
    }
     ...
     * @return 
     */
    function setParm($dir, $filename, $priority = Logs::INFO, $maxlogfilenum = 3, $rotatype = 1, $rotaparam = 5000000)
    {
