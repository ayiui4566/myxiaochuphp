...
class MMysql {
    ...
    protected static $_pdo = null; //静态属性,所有数据库实例共用,避免重复连接数据库
    ...
    protected $_memPort = 11211;
    protected static $_memcache = null;//memcache实例
    public static $production = false;//是否是生成环境 用缓存，测试环境，不用缓存
    protected $isCache = false;//是否缓存
    ...
    public function __construct(array $conf=null) {
        ...
        class_exists('Memcache') or die("Memcache: class not exists.");
        if(!is_null($conf)){
        ...
        if( self::$production && is_null(self::$_memcache)){
            $this->_connectMem();
        }
    }
    private function _connectMem(){
        $_memcache = new Memcache();
        $_memcache->connect($this->_host,$this->_memPort);
    ...
    public static function closeConnection(){
        if(!is_null(self::$_pdo)) self::$_pdo = null;
        if(!is_null(self::$_memcache)) self::$_memcache->close();
    }
    ...
    protected function _connect() {
        ...
        try { 
            $pdo = new PDO($dsn, $this->_user, $this->_pass, $options);
            $pdo->setAttribute(PDO::ATTR_STRINGIFY_FETCHES, false);//提取的时候将数值转换为字符串。
            $pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);  //设置如果sql语句执行错误则抛出异常，事务会自动回滚
            $pdo->setAttribute(PDO::ATTR_EMULATE_PREPARES, false); //禁用prepared statements的仿真效果(防SQL注入)
...
//        var_dump($sql);
        ...
        $pdostmt = self::$_pdo->prepare($this->_sql); //prepare或者query 返回一个PDOStatement
        $pdostmt->execute();
        $result = $pdostmt->fetchAll(PDO::FETCH_ASSOC);

        Logs::getInstance()->pushSqlStr($this->_sql);
        return $result;
    }
    ...
    */
    protected function _doExec($sql='') {
        ...
        Logs::getInstance()->pushSqlStr($this->_sql);
        return self::$_pdo->exec($this->_sql);
    }
...
//        var_dump($sql);
     ...
     * 查询函数
     * @param string $tbName 操作的数据表名
     * @param bool $isCache 这条结果集是否缓存
     * @return array 结果集
     */
    public function select($tbName='',$isCache = false) {
...
//        var_dump($this->_where);
        ...
        //先从memcache服务器中获取数据
        if($this->isCache && self::$production) {
            $key = md5($sql);
            $data = self::$_memcache->get($key);
            if (!$data) {//没有的话则从数据库中查询
                $data = $this->_doQuery(trim($sql));
                self::$_memcache->add($key, $data, MEMCACHE_COMPRESSED, 0);
            } else {
    ...
    /**
     * 设置查询字段
     * @param mixed $field 字段数组 null 表示 select *
     * @return $this
     */
    ...
    */
    public function startTrans() { 
        //数据rollback 支持 
        if ($this->_trans==0) self::$_pdo->beginTransaction();
        $this->_trans++; 
        return; 
    }
